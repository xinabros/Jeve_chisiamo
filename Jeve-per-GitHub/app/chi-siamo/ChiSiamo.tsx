...
function AreaMotif({
  ...
  area: string;
  /** la card d'area lo porta in filigrana, la targhetta del
      responsabile in piccolo e a piena opacità */
  className?: string;
}) {
...
function AreaCard({
  ...
  area: TeamArea;
  index: number;
  /** ⚑ PROVVISORIO — serve alle bozze per spegnere le card non abbinate */
  className?: string;
}) {
  ...
  return (
    <Reveal
      as="li"
      delay={index * 70}
      className={`${styles.teamCell} ${className ?? ""}`}
    >
...
/**
 * Il team — una sezione sola, in due griglie incolonnate.
 *
 * Prima erano due sezioni distinte e distanti: i Responsabili dopo il
 * Board, le Aree più sotto, con in mezzo l'intestazione di sezione. Fra
 * le due c'erano oltre 500px, e nessuno dei modi provati per collegarle
 * — un simbolo condiviso, un filo che scavalcava lo stacco — reggeva,
 * perché rimandava a qualcosa che non era in vista.
 *
 * Qui il legame non si disegna: si vede. Le due griglie hanno la STESSA
 * geometria e stanno una sopra l'altra, quindi ogni responsabile cade
 * nella colonna della sua area; e passando sopra una card qualsiasi
 * resta accesa solo la coppia, mentre le altre otto si spengono.
 *
 * L'indice basta come chiave dell'abbinamento: le due griglie nascono
 * entrambe da `teamAreas`, nello stesso ordine, quindi la colonna n-esima
 * è la stessa area da una parte e dall'altra. Nessuna tabella da tenere
 * allineata a mano.
 */
function Team() {
  const [attiva, setAttiva] = useState<number | null>(null);

  /* L'ascolto sta sulla LISTA, non sulle singole card: un handler per
     griglia invece di dieci, e le card restano ignare del meccanismo. */
  const punta = (e: React.MouseEvent<HTMLUListElement>) => {
    const lista = e.currentTarget;
    const cella = (e.target as HTMLElement).closest("li");
    if (!cella || cella.parentElement !== lista) return;
    setAttiva(Array.prototype.indexOf.call(lista.children, cella));
  };

  const lascia = () => setAttiva(null);

  /* niente di acceso finché il puntatore non entra: a riposo la griglia
     si legge normalmente, l'abbinamento è un di più che si scopre */
    ...
    <section className={styles.teamSection} aria-label="Il team">
      <div className="container">
        ...
        <div className={styles.teamBlock}>
          ...
          <ul
            className={styles.teamLeadsGrid}
            onMouseOver={punta}
            onMouseLeave={lascia}
          >
            {areaLeads.map((lead, i) => (
              /* `lead.role` È il nome dell'area — lo costruisce
                 `areaLeads` qui sopra a partire da `teamAreas` — quindi
                 è già la chiave giusta per `AREA_MOTIFS`. */
              <MemberCard
                key={lead.name}
                member={lead}
                delay={i * 70}
        ...
        {/* Blocco 2 — le aree. Attaccato al precedente: fra i due c'è un
            respiro corto e nessun filetto, perché si leggono come una
            cosa sola e non come due elenchi affiancati. */}
        <div className={`${styles.teamBlock} ${styles.teamBlockAree}`}>
          <Reveal className={styles.uiGroupHead}>
...
function MemberCard({
  ...
  member: BoardMember;
  delay: number;
  /** Nome dell'area, passato solo dai Responsabili. Trasforma la riga
      del ruolo in una targhetta con dentro il simbolo di quell'area:
      lo STESSO tracciato che porta la card dell'area più in basso, non
      un secondo disegno. È così che si legge l'appartenenza fra due
      griglie che stanno in sezioni diverse e distanti.
      Il Board non lo passa: non guida aree, e il suo ruolo resta la
      riga di testo semplice di sempre. */
  area?: string;
  /** ⚑ PROVVISORIO — serve alle bozze per spegnere le card non abbinate */
  className?: string;
}) {
  return (
    <Reveal
      as="li"
      delay={delay}
      className={`${styles.bdCell} ${className ?? ""}`}
    >
      <article className={styles.bdCard}>
        ...
        <div className={styles.bdInfo}>
          ...
          {area ? (
            <p className={`${styles.bdRole} ${styles.bdRolePillWrap}`}>
              <span className={styles.bdRolePill}>
                <AreaMotif area={area} className={styles.bdRoleMotif} />
                {member.role}
              </span>
            </p>
          ) : (
...
/* Ordine di lettura: chi siamo → perché lo facciamo → da quanto →
   chi guida → chi guida le aree → cosa fanno le aree (e l'invito a
   lavorare con noi) → con chi stiamo e cosa abbiamo vinto → cosa ne è
   uscito.

   Prima le aree stavano subito dopo l'hero, e organigramma e
   responsabili in fondo: si conoscevano le squadre prima di sapere
   perché esistono. Ora il racconto scende dall'intento alle persone
   al lavoro che producono. La CTA non è una sezione a sé: chiude
   `Team`, subito sotto le aree, che è il punto in cui ha senso. */
export default function ChiSiamo() {
  return (
    <main>
      ...
      <Timeline />
      <Board />
      {/* i Responsabili non sono più una sezione a sé: stanno dentro
          "Il team", subito sopra le aree che guidano */}
      <Team />
      <Network />
      <AwardsList />
      <Alumni />
