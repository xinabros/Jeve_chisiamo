...
const TEAM_EMPHASIS = {
  hero: styles.teamHero,
  wide: styles.teamWide,
  regular: styles.teamRegular,
} as const;
...
/* Quattro trattamenti visivi alternati: danno varietà al mosaico
   restando dentro la palette del design system. */
...
function AreaCard({ area, index }: { area: TeamArea; index: number }) {
  ...
  const panelId = useId();
  const emphasis = area.emphasis ?? "regular";

  return (
    <Reveal
      as="li"
      delay={index * 70}
      className={`${styles.teamCell} ${TEAM_EMPHASIS[emphasis]}`}
    >
...
/**
 * Team come mosaico editoriale "bento": cinque aree tutte visibili,
 * con pesi visivi differenti (hero / wide / regular) definiti nei dati.
...
/**
 * Board foto-first: cinque card sullo stesso livello, tutte visibili.
