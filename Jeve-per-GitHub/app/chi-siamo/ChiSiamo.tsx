  ...
  return (
    <section className={styles.teamSection} aria-label="Il team">
      <div className="container">
        ...
        {/* Blocco 2 — chi le guida. Stessa card del Board; i nomi arrivano
            da `teamAreas`, non sono riscritti qui, così rinominare qualcuno
            in un posto solo basta. `role` è l'area che guida. */}
        <div className={styles.teamBlock}>
          <Reveal className={styles.uiGroupHead}>
...
/* Quanto è coricato il piano su cui appoggiano gli anelli. A 62° i cerchi
   si leggono come ellissi schiacciate ma restano cerchi: più si alza e
   diventano tondi (si perde la profondità), più si abbassa e collassano
   in righe. */
const NET_PITCH = 62;
/* Ampiezza della parallasse al movimento del mouse, in gradi. */
const NET_TILT = 6;

/* Un anello per livello, dal più ampio al più stretto — lo stesso ordine
   di `networkLayers`.
   `f`  = diametro dell'anello, in frazione del lato del piano.
   `a`  = dove sta il segnalino sulla circonferenza: 0° è il punto più
          vicino a chi guarda, i valori positivi vanno a sinistra. I tre
          angoli sono scelti perché i segnalini non si accavallino.
   `b`  = diametro del tondo che porta il logo.
   `st` = altezza del gambo che lo pianta sull'anello.
   `rw` = spessore dell'anello.

   b, st e rw scalano tutti nello stesso verso: più il livello è ampio,
   più pesa nella figura. È così che si legge la scala — JEurope tiene
   dentro JE Italy, che tiene dentro JEVE — senza doverla annotare. */
const NET_RINGS = [
  { f: 0.88, a: 54, b: 82, st: 38, rw: 2 },
  { f: 0.58, a: -70, b: 62, st: 26, rw: 1.5 },
  { f: 0.29, a: 28, b: 46, st: 16, rw: 1 },
];

/**
 * Network: i tre livelli di appartenenza come cerchi concentrici appoggiati
 * su un piano in prospettiva. Si legge dal centro verso fuori — JEVE sta
 * dentro l'Italia, che sta dentro l'Europa — e la figura dice da sola il
 * "dentro", senza doverlo spiegare.
 *
 * Quarta versione della sezione. Le precedenti erano nodi collegati da
 * curve (che si incrociavano), orbite, e una pila di piani (dove i loghi,
 * coricati sulle lastre, non si leggevano). Qui i cerchi non possono
 * sovrapporsi per costruzione, e i segnalini stanno piantati sull'anello
 * ma dritti verso chi legge, quindi restano sempre leggibili.
 */
function Network() {
      ...
      <div className="container">
        ...
        <div className={styles.netLayout}>
          <div ref={stageRef} className={styles.netStage}>
            <div
              className={styles.netFloor}
              style={{ "--pitch": `${NET_PITCH}deg` } as React.CSSProperties}
            >
              {/* gli anelli: decorativi, li accende il segnalino corrispondente */}
              {networkLayers.map((layer, i) => (
                <span
                  key={`ring-${layer.id}`}
                  aria-hidden="true"
                  className={`${styles.netRing} ${
                    active === layer.id ? styles.netRingOn : ""
                  }`}
                  style={
                    {
                      "--f": NET_RINGS[i].f,
                      "--rw": `${NET_RINGS[i].rw}px`,
                      "--depth": networkLayers.length - i,
                    } as React.CSSProperties
                  }
                />
              ))}

              {/* i segnalini: piantati sull'anello, ma dritti verso chi legge */}
              {networkLayers.map((layer, i) => (
                <a
                  key={layer.id}
                  className={`${styles.netPin} ${
                    active === layer.id ? styles.netPinOn : ""
                  }`}
                  href={layer.href}
          ...
          <div className={styles.netReadout} aria-live="polite">
            {current ? (
              ...
              <p className={styles.netReadoutHint}>
                Tre cerchi concentrici, dal più ampio al più vicino:
                JEurope tiene dentro JE Italy, che tiene dentro JEVE. Più
                il livello è ampio, più pesa nella figura. Passa su un
                livello — o raggiungilo da tastiera — per leggerne il ruolo.
              </p>
            )}
          </div>
        </div>
