...
function Timeline() {
  ...
  /* ==========================================================================
     ...
     poi va da sé: 2019 → 2020 → 2023 → 2024 → 2025, una tappa ogni
  ...
  useEffect(() => {
    ...
    /* `markers` è in ordine DOM, dal più recente: la sequenza va letta
       al contrario, perché il tratto parte dal 2019. */
    const ordinati = [...markers].reverse();
...
/* Un'icona per indicatore, riconosciuta dall'`id` in `impactMetrics`.
   Sono segni, non illustrazioni: stesso tratto, stessa gabbia 24×24. */
const metricIcons: Record<string, ReactNode> = {
  ...
  membri: (
    <>
      ...
      <path d="M14.5 20c0-2.6 1.7-4.8 4-5.6" />
    </>
  ),
  premi: (
    <>
      ...
      <path d="M12 13v4" />
      <path d="M9 21h6l-1-3H10l-1 3z" />
    </>
  ),
};

/**
 * Numero che sale fino al valore finale invece di comparire già scritto.
 *
 * La prima volta parte da 1 quando la sezione entra in vista; ai cambi
 * d'anno riparte dal valore attualmente a schermo, così si vede in che
 * direzione si è mosso. Il conteggio scrive direttamente nel nodo — non
 * passa da uno stato React, che rifarebbe il render a ogni frame.
 */
function useCountUp(target: number, start: boolean) {
  ...
  useEffect(() => {
    ...
    /* la prima salita è più lunga: è quella che si nota */
    const duration = from === 1 ? 1600 : 700;
    let frame = 0;
...
function Metric({
  ...
  const ref = useCountUp(value, start);

  return (
    <Reveal as="li" delay={delay} className={styles.imMetric}>
      <span className={styles.imIcon} aria-hidden="true">
        <svg viewBox="0 0 24 24">{metricIcons[metric.id]}</svg>
      </span>

      {/* il valore finale sta nel markup servito dal server: chi arriva
          senza JS legge il numero giusto, non l'1 di partenza */}
      <p className={styles.imValue}>
...
/**
 * Numeri e testimonianze in un'unica sezione.
 *
 * In alto due colonne: a sinistra l'apertura, a destra il selettore
 * d'anno e i quattro indicatori in card. Sotto, le voci di chi ha
 * lavorato con noi, in un riquadro a due parti — titolo su fondo rosso a
 * sinistra, carosello a destra. Su mobile tutto si impila e si legge di
 * seguito, come una storia breve.
 */
function Impact() {
  const [year, setYear] = useState<ImpactYear>(impactDefaultYear);
  const valori = impactByYear[year];

  /* i numeri partono a salire quando la griglia entra in vista, non al
     caricamento: altrimenti la salita è già finita quando si arriva */
  ...
  /* --- Carosello delle voci: una alla volta, in orizzontale --- */
    ...
    <section className={styles.imSection} aria-label="Impatto e testimonianze">
      <div className="container">
        <div className={styles.imTop}>
          <div className={styles.imTopLeft}>
            <SectionHeader
          ...
          <div className={styles.imTopRight}>
            {/* Selettore d'anno: testo, non pulsanti. Serve a confrontare
                la progressione dei numeri senza aria da cruscotto. */}
            ...
            <ul ref={metricsRef} className={styles.imMetrics}>
              {impactMetrics.map((metric, i) => (
                <Metric
                  key={metric.id}
                  metric={metric}
                  value={valori[metric.id]}
                  start={counting}
                  delay={i * 80}
                />
              ))}
        ...
        {testimonials.length > 0 && (
          <div className={styles.imVoicesWrap}>
            <div className={styles.imVoicesCard}>
              <div className={styles.imVoicesSide}>
                <span className={styles.imVoicesKicker}>
                  {sections.stats.voices.kicker}
                </span>
                <h3 className={styles.imVoicesHeading}>
                  {sections.stats.voices.title}
                </h3>
              </div>
              ...
              <div
                ...
                role="region"
                aria-label={sections.stats.voices.kicker}
              >
                <div
                ...
                >
                  {testimonials.map((item, i) => (
                    /* `figcaption` deve stare in testa o in coda a
                       `figure`: le virgolette decorative, che sono in
                       posizione assoluta, vanno quindi per ultime — a
                       schermo restano comunque in alto a destra. */
                    <figure
                      key={item.quote}
                    ...
                    >
                      <figcaption className={styles.imVoiceInner}>
                        <span className={styles.imVoiceHead}>
                          {item.avatar ? (
                            // eslint-disable-next-line @next/next/no-img-element
                            <img
                      ...
                      <span
                        className={styles.imVoiceQuoteMark}
                        aria-hidden="true"
                      >
            ...
            {/* i comandi restano fuori dal riquadro: con una sola voce
                non servono e spariscono */}
            {totale > 1 && (
              <div className={styles.imVoiceNav}>
                <button
                  type="button"
                  className={styles.imVoiceArrow}
