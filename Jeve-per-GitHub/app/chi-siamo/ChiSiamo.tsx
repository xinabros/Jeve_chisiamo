  ...
  return (
    <Reveal as="li" delay={index * 70} className={styles.teamCell}>
      <article className={`${styles.teamCard} ${open ? styles.teamCardOpen : ""}`}>
        {/* Immagine dell'area: valorizza `photo` in content/chiSiamo.ts
            per sostituire il segnaposto con la fotografia reale. */}
        {area.photo ? (
          // eslint-disable-next-line @next/next/no-img-element
        ...
        {/* occupa tutto lo spazio sopra il blocco di testo e vi centra il
            motivo: si riadatta da solo se il testo cambia altezza */}
        <div className={styles.teamMotifArea}>
          <AreaMotif area={area.name} />
        </div>

        {/* anteprima sempre leggibile: nome, riga di sintesi e CTA */}
        <div className={styles.teamBody}>
          ...
          {/* la descrizione si scopre al passaggio del mouse: a riposo la
              card resta pulita, con nome, chip e CTA */}
          ...
          >
            ...
            <span className={styles.teamMoreIcon} aria-hidden="true">
              {open ? "×" : "+"}
            </span>
          </button>
        </div>
        ...
        {/* Pannello di approfondimento: copre la card per intero e ha una
            propria intestazione con il pulsante di chiusura. Prima era
            ancorato in basso e cresceva oltre la card, finendo per coprire
            il bottone stesso e renderlo incliccabile. */}
        <div id={panelId} className={styles.teamPanel}>
          ...
          <div className={styles.teamPanelBody}>
            ...
            {/* Competenze concrete: dati reali, non chip decorativi ma una
                riga di testo dentro l'approfondimento. */}
            <div className={styles.teamFact}>
              <p className={styles.teamFactLabel}>Di cosa ci occupiamo</p>
              <p className={styles.teamFactText}>
                {area.skills.join(" · ")}
              </p>
            </div>
            ...
            {roster.length > 0 ? (
              ...
              /* finché `people` è vuoto resta il conteggio indicativo:
                 meglio di nomi inventati */
...
function Timeline() {
  ...
  useEffect(() => {
    ...
    const rebuild = () => {
      ...
      /* code corte oltre il primo e l'ultimo marker: la linea non inizia
         né finisce di netto sul pallino */
      const first = points[0];
      const last = points[points.length - 1];
      const extended = [
        { x: first.x, y: Math.max(first.y - 34, 0) },
        ...points,
        { x: last.x, y: Math.min(last.y + 34, box.height) },
      ];
  ...
  useEffect(() => {
    ...
    const update = () => {
      ...
      const from = viewport * 0.85;
      const to = -rect.height + viewport * 0.5;
          ...
          <ol className={styles.tlStages}>
            {events.map((event, i) => (
              <Stage
                ...
                index={i}
                reached={
                  events.length < 2 || progress >= i / (events.length - 1) - 0.04
                }
                markerRef={(node) => {
